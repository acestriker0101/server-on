...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($company_id && $login_id && $password) {
        $db = DB::get();
        // status = 1 (本登録済み) のユーザーのみ。IDまたはメールで検索。
        $stmt = $db->prepare("SELECT * FROM users WHERE company_id = ? AND (login_id = ? OR email = ?) AND status = 1");
        $stmt->execute([$company_id, $login_id, $login_id]);
        $user = $stmt->fetch();
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            $_SESSION['name'] = $user['name']; // ここでDBのnameを保存
            $_SESSION['is_admin'] = $user['is_admin'];
            $_SESSION['plan_rank'] = $user['plan_rank'];

            header("Location: /portal/");
            exit;
        } else {
            $error = "ログインIDまたはパスワードが正しくないか、本登録が完了していません。";
        }
    } else {
...
<body style="background-color: #f7fafc; margin: 0;">
    ...
    <div class="container">
        <div class="auth-container">
            <h2 style="text-align:center; margin-top:0; margin-bottom:30px; color:#2d3748;">ログイン</h2>
            <?php if($error): ?>
                <div class="msg-error"><?= htmlspecialchars($error) ?></div>
            <?php endif; ?>
            <form method="POST">
                <div class="form-group"><label>企業ID</label><input type="text" name="company_id" required></div>
                <div class="form-group"><label>ログインID または メールアドレス</label><input type="text" name="login_id" required></div>
                <div class="form-group"><label>パスワード</label><input type="password" name="password" required></div>
                <button type="submit" class="btn-submit">ログイン</button>
            </form>
            <div class="auth-footer">
                <a href="/portal/forgot">パスワードを忘れた場合</a>
                <span style="color: #cbd5e0;">|</span>
                <a href="/portal/register">新規登録</a>
            </div>
        </div>
    </div>
</body>
