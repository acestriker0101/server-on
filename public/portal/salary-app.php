...
$env = Config::get();
$price_basic = getStripePrice($env['STRIPE_PRICE_SALARY_BASIC'] ?? null, $env['STRIPE_SECRET_KEY'] ?? '');
$price_std = getStripePrice($env['STRIPE_PRICE_SALARY_STANDARD'] ?? null, $env['STRIPE_SECRET_KEY'] ?? '');
$price_pro = getStripePrice($env['STRIPE_PRICE_SALARY_PRO'] ?? null, $env['STRIPE_SECRET_KEY'] ?? '');

 $comparison = [
     ['label' => '月額料金（税込）', 'basic' => $price_basic, 'std' => $price_std, 'pro' => $price_pro],
     ['label' => 'WEB給与明細発行', 'basic' => '✔', 'std' => '✔', 'pro' => '✔'],
     ['label' => '過去履歴の閲覧', 'basic' => '✔', 'std' => '✔', 'pro' => '✔'],
     ['label' => '勤怠データ連携', 'basic' => '—', 'std' => '✔', 'pro' => '✔'],
 ];

$comparison_notes = [
    '* 勤怠データ連携はスタンダード以上のプランで利用可能です。'
];
?>
<!DOCTYPE html>
<html lang="ja">
<head>
    ...
    <style>
        :root { --primary-color: #dd6b20; --bg-dark: #1a202c; }
        .hero-app { 
            background: linear-gradient(135deg, #c05621 0%, #1a202c 100%); 
        ...
        .cta-bottom { background: #fffaf0; padding: 80px 40px; text-align: center; border-radius: 20px; margin-bottom: 60px; border: 1px solid #feebc8; }
...
<body>
    <div class="container" style="max-width: 1200px;">
        ...
        <header class="hero-app">
            <h1>給与計算を、もっと正確に。</h1>
            <p style="font-size: 20px; color: #cbd5e0; max-width: 700px; margin: 0 auto;">勤怠データと連動した自動計算と、スマホで見れるWEB明細。<br>毎月の作業負担を劇的に軽減します。</p>
